...
export default function EditGalleryForm({ id }: EditGalleryFormProps) {
  const router = useRouter();
  const { getRecord: getById, updateRecord } = useSupabase<GalleryItem>(TABLES.GALLERY);
  ...
  useEffect(() => {
    async function loadGalleryItem() {
      ...
      try {
        ...
        setPreviewUrl(item.img_url || null);
        setOriginalImgUrl(item.img_url || null);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        if (originalImgUrl) {
          try {
            await deleteFile(originalImgUrl);
          } catch (err) {
      ...
      setError(err.message || 'Failed to update gallery item. Please try again.');
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="bg-white p-6 rounded-lg shadow-md">
        <div className="text-center py-4">Loading gallery item...</div>
      </div>
    );
  }
